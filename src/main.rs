...
fn rmain(args: &Args) -> Result<(), Error> {
    ...
    wx_adapter(&mut b, &args.target.as_deref().unwrap_or(""))?;
    ...
    replace_wasm_names(out_dir)
}

fn replace_wasm_names(out_dir: &PathBuf) -> Result<(), Error> {
    for entry in std::fs::read_dir(out_dir)? {
        ...
        if let Some(ext) = path.extension() {
            if ext == "js" || ext == "ts" {
                ...
                let new_content = content.replace("WebAssembly", "WXWebAssembly");
                std::fs::File::create(&path)?.write_all(new_content.as_bytes())?;
            }
        }
    }
    Ok(())
...
fn wx_adapter(b: &mut Bindgen, target: &str) -> Result<(), Error> {
    match target {
        "web" => {
            // WeChat does not support network features, and import.meta.url is invalid,
            // so this functionality should be ignored by default under the web target.
            b.omit_default_module_path(true).omit_fetch_module(true);
        }
        _ => {}
    }
